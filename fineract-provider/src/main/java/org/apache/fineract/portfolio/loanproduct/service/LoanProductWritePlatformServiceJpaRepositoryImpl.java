...
import org.apache.fineract.portfolio.loanproduct.repository.RebateRepositoryWrapper;
...
public class LoanProductWritePlatformServiceJpaRepositoryImpl implements LoanProductWritePlatformService {
    ...
    private final RebateRepositoryWrapper rebateRepository;
    ...
    public CommandProcessingResult createLoanProduct(final JsonCommand command) {
        ...
        try {
            ...
            final List<RebatePolicy> rebatePolicy = assembleListOfProductRebates(command);
            ...
            final LoanProduct loanProduct = LoanProduct.assembleFromJson(fund, loanTransactionProcessingStrategyCode, charges, command,
                    this.aprCalculator, floatingRate, rates, loanProductPaymentAllocationRules,rebatePolicy);
    ...
    private List<RebatePolicy> assembleListOfProductRebates(final JsonCommand command) {
        final List<RebatePolicy> rebatePolicy = new ArrayList<>();
        if (command.parameterExists("rebatePolicy")) {
            final JsonArray rebatePolicyArray = command.arrayOfParameterNamed("rebatePolicy");
            if (rebatePolicyArray != null) {
                List<Long> idList = new ArrayList<>();
                for (int i = 0; i < rebatePolicyArray.size(); i++) {

                    final JsonObject jsonObject = rebatePolicyArray.get(i).getAsJsonObject();
                    if (jsonObject.has("id")) {
                        idList.add(jsonObject.get("id").getAsLong());
                    }
                }
                rebatePolicy.addAll(this.rebateRepository.findMultipleWithNotFoundDetection(idList));
            }
        }
